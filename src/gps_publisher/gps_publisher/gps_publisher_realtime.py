...
class GPSPublisher(Node):
    def __init__(self):
        ...
        self.last_msg = None  # Store last valid GPS message
    ...
    def read_nmea_file(self):
        ...
        if not self.filename:
            self.get_logger().warn("No GPS file available!")
            return
        ...
        try:
            with open(self.filename, "r", encoding="utf-8", errors="ignore") as file:
                ...
                self.last_position = file.tell()  # Update last read position

                if not new_data:
                    if self.last_msg:
                        self.publisher_.publish(self.last_msg)
                        self.get_logger().info("🔁 Re-published last GPS data (no new line)")
                    return
                ...
                for line in new_data:
                    ...
                    if line.startswith("$GNGGA"):
                        ...
                        if len(parts) > 9 and parts[2] and parts[4]:  # Ensure valid lat/lon
                            ...
                            self.last_msg = gps_msg  # Cache the last message
