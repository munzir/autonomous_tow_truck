...
class ObstacleDetectionNode(Node):
    def __init__(self):
        ...
        self.model = torch.hub.load('ultralytics/yolov5', 'custom', path='/home/hp/yolov5n.pt', source='local')
        # Grid parameters (size, resolution, and origin in meters)
        self.grid_resolution = 0.1  # 10 cm per cell
        self.grid_size = 100  # 10m x 10m grid
        self.origin_x = -5.0  # Grid origin x in meters
        self.origin_y = -5.0  # Grid origin y in meters
        
        # Buffers to store the latest received frames
    ...
    def image_callback(self, msg):
        """Callback function to receive and store RGB image data."""
        self.latest_color = self.bridge.imgmsg_to_cv2(msg, desired_encoding='bgr8')
        self.process_frame()
    
    def depth_callback(self, msg):
        """Callback function to receive and store depth image data."""
    ...
    def process_frame(self):
        """Processes the latest image and depth data to generate an occupancy grid."""
        if self.latest_color is None or self.latest_depth is None or self.fx is None:
            return  # Ensure all necessary data is available before processing
        
        results = self.model(self.latest_color)  # Run object detection on the image
        grid = np.full((self.grid_size, self.grid_size), -1, dtype=np.int8)  # Initialize occupancy grid
        
        for *box, conf, cls in results.pred[0]:
            x1, y1, x2, y2 = map(int, box)  # Bounding box coordinates
            # center_x, center_y = (x1 + x2) // 2, (y1 + y2) // 2  # Compute object center
            h, w = self.latest_depth.shape
            center_x = np.clip(center_x, 0, w - 1)
            center_y = np.clip(center_y, 0, h - 1)
            depth = float(self.latest_depth[center_y, center_x])
            
            # depth = float(self.latest_depth[center_y, center_x])  # Retrieve depth value at object center
            if np.isnan(depth) or depth <= 0 or depth > 10:
                continue
            elif depth > 0 and depth < 10:  # Ignore objects beyond 10 meters
                ...
                if 0 <= grid_x < self.grid_size and 0 <= grid_y < self.grid_size:
                    grid[grid_y, grid_x] = 100  # Mark obstacle cells with a high occupancy value
        
        self.publish_occupancy_grid(grid)
    
    def publish_occupancy_grid(self, grid):
        """Publishes the generated occupancy grid as a ROS topic."""
        msg = OccupancyGrid()
        ...
        msg.data = grid.flatten().tolist()  # Flatten grid into a 1D list for ROS message
...
def main(args=None):
    """Main function to initialize and run the obstacle detection node."""
